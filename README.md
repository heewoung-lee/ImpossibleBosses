...
* **🤔 문제점:**
    * 잦은 네트워크 파티클 생성/소멸로 인해 가비지 컬렉션(GC)이 빈번하게 발생하여 프레임 드랍을 유발했고, 특히 온라인 환경에서 네트워크 부하가 컸습니다.

* **💡 해결 과정:**
    * **네트워크 오브젝트 재활용 시스템 직접 구축:**
        * 성능 저하 및 끊김 현상(GC) 해결을 위해 오브젝트를 재활용하는 '네트워크 오브젝트 풀링' 시스템을 구현했습니다.
    * **Unity Netcode 기본 동작의 한계 극복:**
        * 기존 Netcode의 기본 생성(Spawn)/소멸(Despawn) 방식은 파티클처럼 자주 사용되는 효과에 비효율적이었습니다.
    * **Netcode 기능 확장: 오브젝트 생성/소멸 방식 직접 제어:**
        * Unity Netcode의 네트워크 오브젝트 생성/소멸 방식을 직접 제어할 수 있는 확장 기능(`INetworkPrefabInstanceHandler`)을 활용했습니다.
        * **맞춤형 관리 로직(커스텀 핸들러) 구현:** 생성 요청 시 풀에서 가져오고, 소멸 요청 시 풀에 반납하도록 처리했습니다.
        * **시스템 연동:** 개발한 '재활용 관리자'를 Netcode 시스템에 등록하여, 네트워크 효과들이 커스텀 재활용 방식을 따르도록 설정했습니다.
    * **자동화된 풀 관리 및 추가 최적화:**
        * 서버(호스트)는 시작 시 풀을 자동 구성하고 효과들을 미리 준비(Prewarm)합니다.
        * 여러 효과 동시 발생 시, 관련 정보를 한 번의 명령으로 처리하도록 추가 최적화를 진행하여 네트워크 통신 부담을 줄였습니다.

* **✨ 개선 결과:**
    * GC 발생 빈도 감소로 게임의 전반적인 프레임 안정성이 향상되었습니다.
    * Netcode 커스터마이징을 통해 네트워크 부하가 줄고, 오브젝트 재사용으로 반응 속도가 개선되었습니다.
    * 멀티플레이 환경에서의 네트워크 오브젝트 생명주기를 효과적으로 제어하여 성능을 최적화했습니다.
