...
* **🤔 문제점:**
    * 초기 테스트 코드 작성 시, 많은 매니저 클래스(`Managers.Instance.SomeManager` 등)나 씬 내 다른 오브젝트의 컴포넌트에 직접 접근하는 컴포넌트 기반의 테스트 방식을 사용했습니다. 이로 인해 테스트 대상 클래스가 특정 씬 구성이나 다른 컴포넌트의 존재에 강하게 의존하게 되어, 단위 테스트(Unit Test) 작성 및 실행이 어렵고, 테스트 환경 설정이 복잡했습니다.
