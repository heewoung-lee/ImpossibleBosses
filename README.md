...
* **NAT Traversal 및 방화벽 문제 해결**:
    * 플레이어의 실제 IP 주소를 노출하지 않고, Unity Relay 서버가 중계자 역할을 수행합니다.
    * 이를 통해 복잡한 네트워크 환경(NAT, 방화벽 등)에서도 별도의 포트 포워딩 설정 없이 플레이어 간 연결을 용이하게 합니다.
* **안정적인 연결 제공**:
    * Unity의 관리형 인프라를 통해 비교적 안정적인 연결 품질을 기대할 수 있습니다.
* **간편한 방 접속**:
    * 호스트는 방 생성 시 고유한 `Join Code`를 발급받습니다.
    * 다른 플레이어들은 이 `JoinCode`만을 사용하여 특정 게임 세션에 쉽게 참여할 수 있습니다.

![Relay Server Flow](https://github.com/user-attachments/assets/4d7df461-dbe3-4e5d-aeb1-72d5b26841ff)
*<p align="center"><sub><strong>&lt;Unity Relay 서버 연결 흐름&gt;</strong><br/>호스트가 Relay Allocation을 받고 Join Code를 생성하면, 클라이언트들은 이 코드를 통해 릴레이 서버에 접속합니다.</sub></p>*
...
* **호스트 마이그레이션 (Host Migration) 준비**:
    * 로비 시스템과 연동하여, 기존 호스트가 게임에서 나가면 새로운 호스트가 릴레이 서버의 Allocation 정보를 이어받아 게임 세션을 계속 유지할 수 있도록 설계되었습니다.
    * 이는 `LobbyManager`에서 호스트 변경 이벤트를 감지하고, 새로운 호스트에게 릴레이 서버 재설정 권한을 부여하는 방식으로 처리됩니다. (관련 로직은 `LobbyManager.cs`의 호스트 변경 콜백 및 `RelayManager.cs`의 호스트/클라이언트 연결 로직에 포함되어 있습니다.)
* **Relay 데이터와 로비 데이터 연동**:
    * 호스트가 릴레이 서버에 성공적으로 방을 할당받으면, 생성된 `Join Code`는 로비 데이터의 일부로 저장됩니다.
    * 이를 통해 다른 클라이언트들이 로비에서 방 정보를 보고, 해당 `Join Code`를 사용하여 릴레이 서버에 접속할 수 있도록 합니다.
* **연결 관리**:
    * `RelayManager` 클래스에서 `StartHostWithRelay` (호스트용) 및 `JoinGuestRelay` (클라이언트용) 메서드를 통해 릴레이 서버 연결 과정을 담당합니다.
    * 연결 해제 및 재연결 로직도 포함하여, 네트워크 상태 변화에 대응합니다.
